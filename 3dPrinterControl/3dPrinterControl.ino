...
void setup() {
  ...
  /*double pos_xy[4][2] = {{50, 0}, {50, 50}, {0, 50}, {0, 0}};
    for(int i=0;i<4;i++){
    p2p_move(pos_xy[i][0], pos_xy[i][1]);`1
    delay(3000);
    }*/
...
void p2p_move_speed(float dest_x, float dest_y, int speed_half_period) {
  ...
  if (diff_x == 0 && diff_y == 0) return;
    ...
    ocr1a_value = 400;
    ocr3a_value = (int)round(abs((400 * (diff_x / diff_y))));
...
void func_btn_enc() {
  ...
  if (enc_btn == 0 && flag_btn == 0) {
    ...
    if (selected_menu_index == SUB_HOME) {
      ...
      while (1) {
        if (motor_x_stop == 1 && motor_y_stop == 1) {
          p2p_move_speed(xy_pos[pos_xy_index][0], xy_pos[pos_xy_index][1], xy_pos[pos_xy_index][2]);
          pos_xy_index++;
          Serial.println(pos_xy_index);
